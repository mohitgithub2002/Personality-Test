...
//   const category = ["Emotional Style", "Communication Approach", "Conflict Resolution", "Relationship Focus"]
    const category = description.map((item)=>item.trait);

  const bgColor = ["#f9dbdc","#F5EDFC", "#FCF2EA", "#E6F2EB", "#E9F1F6","#E9F1F6"];
  const mainColor = ["#9b51e0", "#e0822f", "#0b843e", "#2779a7", "#e0822f"]
  const handelNext=()=>{
    if(index<5){
      setIndex(index+1)
    }
    ...
    <div className=" h-screen overflow-auto flex flex-col items-center justify-center"style={{ backgroundColor:bgColor[index]} }>
      <div className="w-11/12 max-w-4xl mx-auto  bg-white px-10 py-4 m-4 md:h-3/5">
        <div className="flex justify-between items-center">
          ...
          <button className="bg-myblack text-white px-6 py-2 rounded" onClick={handelNext} disabled={index===5}>Next</button>
        </div>
        {index===0?
          <div className="mt-4 md:mt-6">
            <h2 className="text-2xl font-quicksand font-semibold text-myblack text-center py-4" >{personality[3]}</h2>
            ...
            <img  />
            <div className="mt-4 text-center text-gray-600 ">{personality[1]}</div>
          </div>
        :
          <div className="mt-6 md:mt-8">
              ...
              <div className='flex justify-between'>
                ...
                <div className='text-myblack'>100%</div>
              </div>
              {/* <div className='flex justify-between'>
              ...
              <div className="mt-4 text-center text-gray-600">{description[index-1].description}</div>
            </div>
        }
...
const SurveyPage = ({params}) => {
  ...
  const fetchData = async()=>{
    ...
    const about = "You are average in communication"
    const data = await res.json();
    setResult(data.data);
    setScores(data.data.categoryScore);
    setDescription(data.data.traits)
    setPersonality([data.data.score, about, data.data.title])
